                ...
                <div class="col-md-6">
                    ...
                    <table>
                        <tr>
                            <td>Status</td>
                            <td class="px-2">:</td>
                            <td>@include('includes.badge-status', ['status' => $pembelian->status])</td>
                        </tr>
                        <tr>
                            <td>Date</td>
                            <td class="px-2">:</td>
                        ...
                        <tr>
                            <td>Total Amount</td>
                            <td class="px-2">:</td>
                            <td class="rupiah-format">{{ $pembelian->total_amount }}</td>
                        </tr>
                    </table>
                </div>
                <div class="col-md-6">
                    <div class="d-flex justify-content-end" style="gap: 0.5rem">
                        @if ($pembelian->status !== 'DONE')
                        ...
                        @endif
                        @if ($pembelian->status == 'WAITING_VALIDATION' && request()->session()->get('user')['role'] === 'KEPALA_KLINIK')
                            <form action="{{ route('pembelian.update', $pembelian->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                    ...
                    <div class="card">
                        <div class="card-body">
                            @if ($pembelian->status == 'DRAFT')
                                ...
                                @include('pages.pembelian.item.create')
                            @endif
                            <table id="defaultTable" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($items as $item)
                                            ...
                                            @if ($pembelian->status == 'DRAFT')
                                                <td>
                                                    ...
                                                    <form id="formDelete{{ $item->id }}"
                                                        action="{{ route('pembelian-item.destroy', $item->id) }}"
                                        ...
                                        @include('pages.pembelian.item.update')
                                        <?php $i++; ?>
                                    @endforeach
                                </tbody>
                            </table>
